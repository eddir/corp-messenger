...
public interface MessageRepository extends JpaRepository<Message, Long>
{
    ...
    @Query(value = "select * from message m join chat c on m.chat_id = c.id WHERE c.id = :chatId ORDER BY m.id LIMIT :count", nativeQuery = true)
    public List<Message> getMessageByChatByCountAntIntervalDefaultOrd(Long chatId, Long count);

    @Query(value = "select * from message m join chat c on m.chat_id = c.id WHERE c.id = :chatId ORDER BY m.id DESC LIMIT :count", nativeQuery = true)
    public List<Message> getMessageByChatByCountAntIntervalInverseOrd(Long chatId, Long count);
